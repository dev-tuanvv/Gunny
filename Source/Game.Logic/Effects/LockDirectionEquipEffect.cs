namespace Game.Logic.Effects
{
    ...
    public class LockDirectionEquipEffect : BasePlayerEffect
    {
        ...
        private void ChangeProperty(Player player)
        {
            if (!player.CurrentBall.IsSpecial())
            {
                ...
                if ((base.rand.Next(100) < this.m_probability) && (player.AttackGemLimit == 0))
                {
                    ...
                    player.Game.AddAction(new LivingSayAction(player, LanguageMgr.GetTranslation("LockDirectionEquipEffect.msg", new object[0]), 9, 0, 0x3e8));
                }
            }
        }
        ...
        {
            LockDirectionEquipEffect ofType = living.EffectList.GetOfType(eEffectType.LockDirectionEquipEffect) as LockDirectionEquipEffect;
            if (ofType != null)
            {
