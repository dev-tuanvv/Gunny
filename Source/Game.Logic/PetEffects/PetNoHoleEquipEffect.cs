namespace Game.Logic.PetEffects
{
    ...
    public class PetNoHoleEquipEffect : BasePetEffect
    {
        ...
        public PetNoHoleEquipEffect(int count, int probability, int type, int skillId, int delay, string elementID) : base(ePetEffectType.NoHoleEquipEffect, elementID)
        {
            this.m_count = count;
        ...
        private void player_AfterKilledByLiving(Living living)
        {
            if ((base.rand.Next(0x2710) < this.m_probability) && (living.PetEffects.CurrentUseSkill == this.m_currentId))
            {
                ...
                new PetNoHoleEffect(this.m_count, base.Info.ID.ToString()).Start(living);
            }
        }
