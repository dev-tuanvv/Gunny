namespace Game.Logic.PetEffects
{
    ...
    public class PetAddBloodForSelfEquipEffect : BasePetEffect
    {
        ...
        public PetAddBloodForSelfEquipEffect(int count, int probability, int type, int skillId, int delay, string elementID) : base(ePetEffectType.PetAddBloodForSelfEquipEffect, elementID)
        {
            this.m_count = count;
        ...
        private void player_AfterKilledByLiving(Living living)
        {
            if ((base.rand.Next(0x2710) < this.m_probability) && (living.PetEffects.CurrentUseSkill == this.m_currentId))
            {
                ...
                new PetAddBloodForSelfEquip(this.m_count, this.m_currentId, base.Info.ID.ToString()).Start(living);
            }
        }
