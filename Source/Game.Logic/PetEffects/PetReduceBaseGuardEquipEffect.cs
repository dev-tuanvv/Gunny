namespace Game.Logic.PetEffects
{
    ...
    public class PetReduceBaseGuardEquipEffect : BasePetEffect
    {
        ...
        public PetReduceBaseGuardEquipEffect(int count, int probability, int type, int skillId, int delay, string elementID) : base(ePetEffectType.PetReduceBaseGuardEquipEffect, elementID)
        {
        ...
        private void player_AfterKilledByLiving(Living living)
        {
            if ((base.rand.Next(0x2710) < this.m_probability) && (living.PetEffects.CurrentUseSkill == this.m_currentId))
            {
                ...
                new PetReduceBaseGuardEquip(this.m_count, this.m_currentId, base.Info.ID.ToString()).Start(living);
            }
        }
