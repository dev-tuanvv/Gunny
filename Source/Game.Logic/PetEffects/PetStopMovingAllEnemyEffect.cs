namespace Game.Logic.PetEffects
{
    ...
    public class PetStopMovingAllEnemyEffect : BasePetEffect
    {
        ...
        public PetStopMovingAllEnemyEffect(int count, int probability, int type, int skillId, int delay, string elementID) : base(ePetEffectType.PetStopMovingAllEnemyEffect, elementID)
        {
            this.m_count = count;
        ...
        private void player_AfterKilledByLiving(Living living)
        {
            if ((base.rand.Next(0x2710) < this.m_probability) && (living.PetEffects.CurrentUseSkill == this.m_currentId))
            {
                ...
                {
                    player.AddPetEffect(new PetStopMovingEquip(this.m_count, base.Info.ID.ToString()), 0);
                }
            }
        }
