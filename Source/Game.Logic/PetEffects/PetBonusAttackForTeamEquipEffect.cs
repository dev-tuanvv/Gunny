namespace Game.Logic.PetEffects
{
    ...
    public class PetBonusAttackForTeamEquipEffect : BasePetEffect
    {
        ...
        public PetBonusAttackForTeamEquipEffect(int count, int probability, int type, int skillId, int delay, string elementID) : base(ePetEffectType.PetBonusAttackForTeamEquipEffect, elementID)
        {
            this.m_count = count;
        ...
        private void ChangeProperty(Player player)
        {
            if ((base.rand.Next(100) < this.m_probability) && (player.PetEffects.CurrentUseSkill == this.m_currentId))
            {
                ...
                {
                    player2.AddPetEffect(new PetAddAttackEquip(this.m_count, this.m_currentId, base.Info.ID.ToString()), 0);
                }
