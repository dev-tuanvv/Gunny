namespace Game.Logic.PetEffects
{
    ...
    public class PetReduceBaseDamageTargetEquipEffect : BasePetEffect
    {
        ...
        public PetReduceBaseDamageTargetEquipEffect(int count, int probability, int type, int skillId, int delay, string elementID) : base(ePetEffectType.PetReduceBaseDamageTargetEquipEffect, elementID)
        {
            this.m_count = count;
        ...
        {
            if (base.IsTrigger && (living != target))
            {
                target.AddPetEffect(new PetReduceBaseDamageEquip(this.m_count, this.m_currentId, base.Info.ID.ToString()), 0);
                base.IsTrigger = false;
            }
        }
