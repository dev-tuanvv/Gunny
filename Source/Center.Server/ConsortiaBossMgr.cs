    ...
    public sealed class ConsortiaBossMgr
    {
        ...
        public static List<RankingPersonInfo> SelectRank(int consortiaId)
        {
            ...
            try
            {
                ...
                IOrderedEnumerable<KeyValuePair<string, RankingPersonInfo>> enumerable = from pair in m_consortias[consortiaId].RankList
                    orderby pair.Value.TotalDamage descending
                    select pair;
        ...
        public static void UpdateBlood(int consortiaId, int damage)
        {
            ...
            try
            {
                if (m_consortias.ContainsKey(consortiaId))
                {
                    ...
                    {
                        CloseConsortia(consortiaId, true);
                    }
                }
            }
            finally
        ...
        public static void UpdateRank(int consortiaId, int damage, int richer, int honor, string nickName, int UserID)
        {
            ...
            try
            {
                if (m_consortias.ContainsKey(consortiaId))
                {
                    ...
                    {
                        RankingPersonInfo local1 = m_consortias[consortiaId].RankList[nickName];
                        local1.TotalDamage += damage;
                        RankingPersonInfo local2 = m_consortias[consortiaId].RankList[nickName];
                        local2.Damage += richer;
                        RankingPersonInfo local3 = m_consortias[consortiaId].RankList[nickName];
                        local3.Honor += honor;
                    }
                    else
                    {
        ...
        public static void UpdateTime()
        {
            foreach (ConsortiaInfo info in m_consortias.Values)
            {
                ...
                {
                    CloseConsortia(info.ConsortiaID, false);
                }
