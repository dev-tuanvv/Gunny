    ...
    public class QuestInventory
    {
        ...
        public void LoadFromDatabase(int playerId)
        {
            ...
            try
            {
                ...
                List<BaseQuest> arg_AD_0 = this.m_list;
            }
            finally
            {
                Monitor.Exit(@lock);
        ...
        public bool Finish(BaseQuest baseQuest, int selectedItem)
        {
            ...
            try
            {
                if (baseQuest.Finish(this.m_player))
                {
                    ...
                    foreach (QuestAwardInfo current in questGoods)
                    {
                        if (!current.IsSelect || current.RewardItemID == selectedItem)
                        {
                            ...
                            if (itemTemplateInfo != null)
                            {
                                ...
                                int num = current.RewardItemCount;
                                if (current.IsCount)
                                {
                                    num *= data.RandDobule;
                                }
                                ...
                                {
                                    int count = (i + itemTemplateInfo.MaxCount > current.RewardItemCount) ? (current.RewardItemCount - i) : itemTemplateInfo.MaxCount;
                                    ...
                                    if (itemInfo != null)
                                    {
                                        ...
                                        {
                                            if (itemTemplateInfo.BagType == eBageType.FarmBag)
                                            {
                                                list3.Add(itemInfo);
                                            }
                                            else
                                            {
                                                if (itemTemplateInfo.BagType == eBageType.BeadBag)
                                                {
                                                    list4.Add(itemInfo);
                                                }
                                                else
                    ...
                    if (list.Count > 0 && this.m_player.EquipBag.GetEmptyCount() < list.Count)
                    {
                        baseQuest.CancelFinish(this.m_player);
                        this.m_player.Out.SendMessage(eMessageType.ERROR, this.m_player.GetInventoryName(eBageType.EquipBag) + LanguageMgr.GetTranslation("Game.Server.Quests.BagFull", new object[0]) + " ");
                        bool result = false;
                        return result;
                    }
                    ...
                    if (list3.Count > 0 && this.m_player.FarmBag.GetEmptyCount() < list3.Count)
                    {
                        baseQuest.CancelFinish(this.m_player);
                        this.m_player.Out.SendMessage(eMessageType.ERROR, this.m_player.GetInventoryName(eBageType.FarmBag) + LanguageMgr.GetTranslation("Game.Server.Quests.BagFull", new object[0]) + " ");
                        bool result = false;
                        return result;
                    }
                    if (list4.Count > 0 && this.m_player.BeadBag.GetEmptyCount() < list4.Count)
                    {
                        baseQuest.CancelFinish(this.m_player);
                        this.m_player.Out.SendMessage(eMessageType.ERROR, this.m_player.GetInventoryName(eBageType.BeadBag) + LanguageMgr.GetTranslation("Game.Server.Quests.BagFull", new object[0]) + " ");
                        bool result = false;
                        return result;
                    }
                    foreach (ItemInfo current2 in list)
                    {
                        if (!this.m_player.EquipBag.StackItemToAnother(current2) && !this.m_player.EquipBag.AddItem(current2))
                        {
                            list5.Add(current2);
                        }
                    }
                    ...
                    foreach (ItemInfo current5 in list4)
                    {
                        if (!this.m_player.BeadBag.AddItem(current5))
                        {
                            list5.Add(current5);
                        }
                    }
                    if (list5.Count > 0)
                    {
                        this.m_player.SendItemsToMail(list5, "Bagfull trả về thư!", "Phần thưởng nhiệm vụ!", eMailType.ItemOverdue);
