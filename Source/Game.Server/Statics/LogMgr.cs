namespace Game.Server.Statics
{
    ...
    public class LogMgr
    {
        ...
        public static void LogDropItemAdd(int userId, int itemId, int templateId, int dropId, int dropData)
        {
            try
            {
                object[] values = new object[] { _gameType, _areaId, _serverId, userId, itemId, templateId, dropId, dropData, DateTime.Now };
                lock (m_LogDropItem)
                {
        ...
        public static void LogFightAdd(int roomId, eRoomType roomType, eGameType fightType, int changeTeam, DateTime playBegin, DateTime playEnd, int userCount, int mapId, string teamA, string teamB, string playResult, int winTeam, string BossWar)
        {
            try
            {
                object[] values = new object[] { _gameType, _serverId, _areaId, roomId, (int) roomType, (int) fightType, changeTeam, playBegin, playEnd, userCount, mapId, teamA, teamB, playResult, winTeam, BossWar };
                lock (m_LogFight)
                {
        ...
        public static void LogItemAdd(int userId, LogItemType itemType, string beginProperty, SqlDataProvider.Data.ItemInfo item, string AddItem, int result)
        {
            try
            {
                string str = "";
                if (item != null)
                {
                    str = string.Format("{0},{1},{2},{3},{4},{5},{6},{7},{8}", new object[] { item.StrengthenLevel, item.Attack, item.Defence, item.Agility, item.Luck, item.AttackCompose, item.DefendCompose, item.AgilityCompose, item.LuckCompose });
                }
                object[] values = new object[] { _gameType, _serverId, _areaId, DateTime.Now, userId, (int) itemType, (item == null) ? "" : item.Template.Name, (item == null) ? 0 : item.ItemID, AddItem, beginProperty, str, result };
                lock (m_LogItem)
                {
        ...
        public static void LogMoneyAdd(LogMoneyType masterType, LogMoneyType sonType, int userId, int moneys, int SpareMoney, int gold, int giftToken, int offer, string otherPay, string goodId, string goodsType)
        {
            try
            {
                if ((moneys != 0) && (moneys <= SpareMoney))
                {
                    ...
                    object[] objArray = new object[] { _gameType, _serverId, _areaId, masterType, sonType, userId, DateTime.Now, moneys, SpareMoney, gold, giftToken, offer, otherPay, goodId, goodsType };
                    lock (m_LogMoney)
                    {
                    }
                }
        ...
        public static void LogMoneyAdd(LogMoneyType masterType, LogMoneyType sonType, int userId, int moneys, int SpareMoney, int gold, int giftToken, int offer, int medal, string otherPay, string goodId, string goodsType)
        {
            ...
            Label_0105:
                moneys *= -1;
            Label_010A:;
                object[] values = new object[] { _gameType, _serverId, _areaId, masterType, sonType, userId, DateTime.Now, moneys, SpareMoney, gold, giftToken, offer, medal, otherPay, goodId, goodsType };
                lock (m_LogMoney)
                {
