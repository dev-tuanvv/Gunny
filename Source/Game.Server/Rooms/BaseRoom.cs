...
namespace Game.Server.Rooms
{
	public class BaseRoom
	{
        ...
        public bool StartWithNpc
        {
            get
            {
                return this.m_startWithNpc;
            }
            set
            {
                this.m_startWithNpc = value;
            }
        }
		...
		public void UpdateRoomGameType()
		{
			...
			case eRoomType.FightGround:
			case eRoomType.TransnationalFight:
				goto IL_D0;
			case eRoomType.BattleRoom:
				this.GameType = eGameType.BattleGame;
				return;
		...
		public void SendRoomSetupChange(BaseRoom room)
		{
			if (this.m_host != null)
			{
				GSPacketIn pkg = this.m_host.Out.SendGameRoomSetupChange(room);
				this.SendToAll(pkg);
			}
		}
			...
			if (num != -1)
			{
				...
				foreach (GamePlayer current in players)
				{
					if (current != player)
					{
						...
						player.Out.SendBufferList(current, current.BufferList.GetAllBufferByTemplate());
					}
				}
                if (this.m_host == null)
                {
			...
			if (num != -1)
			{
				...
				else
				{
					...
					if (flag)
					{
						...
						{
							current2.Out.SendGameRoomSetupChange(this);
						}
					}
				}
			}
		...
		public void RemoveAllPlayer()
		{
			for (int i = 0; i < 10; i++)
			{
				if (this.m_places[i] != null)
				{
					...
					RoomMgr.AddAction(new EnterWaitingRoomAction(this.m_places[i]));
				}
			}
		}
