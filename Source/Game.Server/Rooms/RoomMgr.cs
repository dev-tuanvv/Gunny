...
namespace Game.Server.Rooms
{
	public class RoomMgr
	{
		...
		private static BaseSevenDoubleRoom[] m_sevenDoubleRoom;
		private static BaseCampBattleRoom m_campBattleRoom;
		private static BaseConsBatRoom m_ConsBatRoom;
		...
		public static BaseSevenDoubleRoom[] SevenDoubleRoom
		{
			get
			{
				return RoomMgr.m_sevenDoubleRoom;
			}
		}
		public static BaseCampBattleRoom CampBattleRoom
		{
			get
			{
				return RoomMgr.m_campBattleRoom;
			}
		}
		public static BaseConsBatRoom ConsBatRoom
		{
			get
			{
				return RoomMgr.m_ConsBatRoom;
			}
		}
		public static BaseChristmasRoom ChristmasRoom
		{
			get
			{
				return RoomMgr.m_christmasRoom;
			}
		}
		...
		public static bool Setup(int maxRoom)
		{
			...
			RoomMgr.m_sevenDoubleRoom = new BaseSevenDoubleRoom[maxRoom];
			for (int j = 0; j < maxRoom; j++)
			{
				RoomMgr.m_sevenDoubleRoom[j] = new BaseSevenDoubleRoom(j + 1);
			}
			...
			RoomMgr.m_ConsBatRoom = new BaseConsBatRoom();
			RoomMgr.m_campBattleRoom = new BaseCampBattleRoom();
			return true;
		}
		...
		{
			RoomMgr.AddAction(new CreateCampBattleRoomAction(player, ChallengePlayer));
		}
		public static void CreateCampBattleBossRoom(GamePlayer player, int mapId)
		{
			RoomMgr.AddAction(new CreateCampBattleBossAction(player, eRoomType.CampBattleBattle, 1, mapId));
		}
		...
		{
			RoomMgr.AddAction(new CreateConsortiaBattleRoomAction(player, ChallengePlayer));
		}
		public static void CreateBattleRoom(GamePlayer player, eRoomType roomType)
		{
			RoomMgr.AddAction(new CreateBattleRoomAction(player, roomType));
		}
		public static void CreateFightFootballTimeRoom(GamePlayer player, eRoomType roomType)
		{
			RoomMgr.AddAction(new CreateFightFootballTimeRoomAction(player, roomType));
		}
		public static void CreateEncounterRoom(GamePlayer player, eRoomType roomType)
		{
			RoomMgr.AddAction(new CreateEncounterRoomAction(player, roomType));
		}
		public static void CreateGroupBattleRoom(GamePlayer player, int groupType)
		{
			RoomMgr.AddAction(new CreateGroupBattleRoomAction(player, groupType));
		}
		...
		public static void PickupNpc(int roomId)
		{
			RoomMgr.AddAction(new PickupNpcAction(roomId));
		}
		public static void UpdateRoomGameType(BaseRoom room, eRoomType roomType, byte timeMode, eHardLevel hardLevel, int levelLimits, int mapId, string password, string roomname, bool isCrosszone)
		{
			RoomMgr.AddAction(new RoomSetupChangeAction(room, roomType, timeMode, hardLevel, levelLimits, mapId, password, roomname, isCrosszone));
		}
		...
		public static List<BaseRoom> GetAllMatchRooms(eRoomType type)
		{
			List<BaseRoom> list = new List<BaseRoom>();
			BaseRoom[] rooms;
			Monitor.Enter(rooms = RoomMgr.m_rooms);
			try
			{
				...
				{
					BaseRoom baseRoom = rooms2[i];
					if (!baseRoom.IsEmpty && baseRoom.RoomType == type)
					{
						list.Add(baseRoom);
					}
				}
