...
namespace Game.Server.Rooms
{
	internal class EnterRoomAction : IAction
	{
		...
		public void Execute()
		{
			...
			if (baseRoom != null && baseRoom.RoomType == eRoomType.ActivityDungeon && this.m_player.Actives.Info.activityTanabataNum > 0)
			{
				this.m_player.Out.SendMessage(eMessageType.Normal, "Bạn đã tham gia hôm nay!");
				return;
			}
			...
			if (!baseRoom.NeedPassword || baseRoom.Password == this.m_pwd)
			{
				if (baseRoom.Game == null || baseRoom.Game.CanAddPlayer())
				{
					...
					this.m_player.Out.SendGameRoomSetupChange(baseRoom);
					return;
				}
			}
			else
