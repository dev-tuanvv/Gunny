...
namespace GameServerScript.AI.NPC
{
    public class FiveNormalThirdBoss : ABrain
    {
        ...
        private int m_npcId1 = 5122; //mo han

        private int m_npcId2 = 5123; // hop quai di

        private int m_npcId3 = 5124; // dai` cao

        private int m_npcEnemyId = 5104; // lao gia cho ma
        ...
        private void CreateNpc2()
        {
            ...
            m_npc2 = ((SimpleBoss)Body).CreateChild(m_npcId2, m_pointCreateNpc2[createPoint].X, m_pointCreateNpc2[createPoint].Y, true, ((PVEGame)Game).BaseLivingConfig());
        ...
        {
            Body.Say("Thôi ta éo đùa nữa.", 0, 500);
            Body.PlayMovie("out", 500, 0);
            Body.CallFuction(new LivingCallBack(CreateNpcEndGame), 3500);
        }

        private void CreateNpcEndGame()
        {
            SimpleNpc npc = ((PVEGame)Game).CreateNpc(m_npcEnemyId, 0, 0, 1, 1, "standC", ((PVEGame)Game).BaseLivingConfig());
