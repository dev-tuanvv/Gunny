...
namespace GameServerScript.AI.NPC
{
    public class SimpleBomblingNpc : ABrain
    {
        ...
        {
            int dis = Game.Random.Next(((SimpleNpc)Body).NpcInfo.MoveMin - 100, ((SimpleNpc)Body).NpcInfo.MoveMax - 100);
            if (player.X > Body.X)
            {
                Body.MoveTo(Body.X + dis, Body.Y, "walk", 2000, "", 4, new LivingCallBack (Beat));
            }
            else
            {
                Body.MoveTo(Body.X - dis, Body.Y, "walk", 2000, "", 4, new LivingCallBack (Beat));
