...
namespace GameServerScript.AI.NPC
{
    public class NormalKingSecond : ABrain
    {
        ...
        public override void OnStartAttacking()
        {
            ...
            //{
            //    if (((SimpleBoss)Body).Child.Count < 9)
            //    {
            //        CreateChild();
            //        //ProtectingWall();
            //        m_attackTurn++;
        ...
        private void ProtectingWall()
        {
            if (IsEixt == 0)
            {
                m_wallLeft = ((PVEGame)Game).CreatePhysicalObj(Body.X - 40, 600, "wallLeft", "com.mapobject.asset.WaveAsset_01_left", "1", 1, 0);
                m_wallRight = ((PVEGame)Game).CreatePhysicalObj(Body.X + 40, 600, "wallLeft", "com.mapobject.asset.WaveAsset_01_right", "1", 1, 0);
                m_wallLeft.SetRect(-165, -169, 3, 330);
                m_wallRight.SetRect(128, 165, 3, 330);
                IsEixt = 1;
            }
        ...
        {
            ((SimpleBoss)Body).CreateChild(npcID, 670, 680, -1, 400, 9);
        }
